...
To install WSL open PowerShell as Administrator or Windows Terminal as Administrator and run:
...
git clone https://github.com/BeeSafeRobotics/csci_420_robotics_docker.git
```
...
git clone https://github.com/BeeSafeRobotics/csci_420_robotics_labs_f26.git
docker compose up --build -d
```

The `docker compose up --build -d` command looks in the [`docker compose.yml`](https://github.com/BeeSafeRobotics/csci_420_robotics_docker/blob/main/docker_compose.yml)
...
* `--build` builds or rebuilds the Docker images before starting the containers.
* `-d` runs the containers in detached mode, allowing them to continue running in the background while you use your terminal.
...
```
### Starting and Restarting the Docker Container

After the containers have been created, you do not need to rebuild them every time you want to use the lab environment. Which command you use depends on what you want to do:

* `docker compose stop` stops the running containers but keeps them so they can be restarted later.
* `docker compose start` starts existing containers that were previously stopped.
* `docker compose rm` removes stopped containers. After removing them, you will need to use `docker compose up -d` to create and start them again.
* `docker compose up -d` creates and starts the containers. Use this if the containers do not exist or after changing `docker compose.yml`.
* `docker compose up --build -d` rebuilds the Docker images before creating and starting the containers. Use this after changing the `Dockerfile` or when the images need to be rebuilt.

For normal use after stopping the containers, simply run:

```bash
cd ~/csci_420_robotics_docker
...
Throughout the semester the teaching staff will be adding the labs to the [lab GitHub repository](https://github.com/BeeSafeRobotics/csci_420_robotics_labs_f26) during the week the lab is assigned.
